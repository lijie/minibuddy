...
### D7-1：字符级 Token 估算而非引入 tiktoken

**决策**：用简单规则估算 token 数（中文 2 字符 ≈ 1 token，ASCII 4 字符 ≈ 1 token），不引入 `tiktoken-rs`。

**原因**：
- 教学项目，避免引入重量级依赖（tiktoken-rs 需要下载 tokenizer 数据文件）
- 估算误差 ±20% 对截断决策影响不大——留了足够 buffer（max_tokens 默认 8000，多数模型支持 32K+）
- 精确计数需要知道具体 tokenizer（各模型不同），简单估算是唯一通用方案

---

### D7-2：保留 system prompt + 最近消息的截断策略

**决策**：超限时保留 `messages[0]`（system prompt）+ 从后往前尽可能多的最近消息，删除中间的旧消息。

**原因**：
- System prompt 定义了 Agent 行为和工具列表，必须始终保留
- 最近的消息对当前对话最相关（用户刚问的问题、刚获得的工具结果）
- 最旧的对话轮次对当前任务帮助最小，优先丢弃
- 比"摘要旧消息"方案简单得多（摘要需要额外 LLM 调用，增加延迟和成本）

---

### D7-3：JSON 文件持久化而非 SQLite

**决策**：每个会话存为一个 JSON 文件 `~/.mini-buddy/sessions/{timestamp}.json`，不使用 SQLite。

**原因**：
- 零额外依赖（serde_json 已有）
- 每文件独立，可直接用文本编辑器查看/调试
- 教学透明：学习者能直接 `cat` 查看存储格式
- 个人使用场景下会话数不会太多，不需要数据库的索引查询能力

**代价**：无法高效搜索历史对话内容（需遍历所有文件）。对教学项目可接受。

---

### D7-4：斜杠命令通过 UserAction::Command 路由

**决策**：TUI 中输入以 `/` 开头的文本作为命令处理（`/save`、`/load`、`/new`），走 `UserAction::Command` 分支而非 `Submit`。
